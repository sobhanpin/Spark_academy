...
export default function StudentDashboard() {
  ...
  const deleteFile = async (u: Upload) => {
    const { error: storageErr } = await supabase.storage.from('uploads').remove([u.file_path])
  ...
  const updateProfile = async (form: FormData) => {
    if (!session) return
    const { error } = await supabase.from('profiles').update({ name: form.get('name') as string, phone: form.get('phone') as string }).eq('id', session.user.id)
    if (error) { showToast('خطا در ذخیره پروفایل: ' + error.message, 'error'); return }
