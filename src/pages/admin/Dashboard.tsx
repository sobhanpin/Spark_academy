  ...
  if (editing) {
    return (
      <div className="card space-y-3">
        ...
        <div>
          <label className="block text-xs text-[#7B7FB5] mb-1">عکس دوره</label>
          {editing.image_url && <img src={editing.image_url} alt="پیش‌نمایش" className="w-full h-32 object-cover rounded-lg mb-2" />}
          <input type="file" accept="image/*" disabled={imgBusy} onChange={(e) => e.target.files?.[0] && uploadImage(e.target.files[0])} className="input" />
...
function TeachersTab() {
  ...
  const addTeacher = async () => {
    ...
    if (error || !data || data.length === 0) { alert('کاربری با این ایمیل پیدا نشد.'); return }
    setEmail(''); load()
  }
